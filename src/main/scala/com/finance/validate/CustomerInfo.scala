...
object CustomerInfo {
  ...
  def addCustomer(user: Customer)(using Resource[IO, Client[IO]], Resource[IO, HikariTransactor[IO]], Logger[IO]): IO[Either[ClientErrorMessage, Int]] = {
    val psVal = validatePostCode(user.postCode)
    for {
      a <- psVal
      newClient = createNewClient(Customer(user.firstName, user.lastName, user.emailAddress, user.firstLineAddress, a.region, a.postCode, user.age, user.creditScore))
      output <- addNewUser(newClient)
    } yield output

  }

  def fetchCustomer(userId: String)(using Resource[IO, HikariTransactor[IO]]): IO[Either[ClientErrorMessage, ExistingClient]] = {
    fetchUser(userId)
  }
