...
import com.dimafeng.testcontainers.{DockerComposeContainer, ExposedService, ForAllTestContainer, KafkaContainer, MultipleContainers, WaitingForService}
...
class Ingestion extends AnyFlatSpec with ForAllTestContainer with Matchers with BeforeAndAfterAll {

  override val container: DockerComposeContainer = DockerComposeContainer(
    Seq(new File("docker-compose.yaml")),
    waitingFor = Some(WaitingForService("kafka1", Wait.forLogMessage(".*[Kafka Server is running.]\n", 1))),
    exposedServices = Seq(
      ExposedService("kafka1", 29092, Wait.forLogMessage(".*[Kafka Server is running.]\n", 1)),
      ExposedService("kafka2", 29093, Wait.forLogMessage(".*[Kafka Server is running.]\n", 1)),
      ExposedService("kafka3", 29094, Wait.forLogMessage(".*[Kafka Server is running.]\n", 1))
    )
  )
  ...
  val producerResource: Resource[IO, KafkaProducer[IO, String, String]] = KafkaProducer[IO].resource(producerSettings)
  ...
  producerResource.use{ producer =>
    for{
      ...
      _ <- consumeTransfers()(using consumerSettings, producerSettings, logger).timeout(15.seconds).handleErrorWith{
